...
export default function Board({
  ...
  const getStyle = (squareName: string) => ({
  ...
  const squareElements = board.pieces.flat().map((pieceName, index) => {
    ...
    const selectedIndex = selectedPosition
      ? selectedPosition?.x + selectedPosition?.y * 8
      : null;
    ...
    const isAvailableMove = availableMoves.some(
      (move) => move.to === index && move.from === selectedIndex
    );
    const isAvailableCapture = availableMoves.some(
      (move) => move.to === index && move.from === selectedIndex && move.capture
    );
    ...
    return (
      <div
        ...
        style={getStyle(squareName)}
      >
        {pieceElement}
        {isSelected ? <div className="square__selected" /> : null}
        {isAvailableMove && !isAvailableCapture ? (
          <div className="square__available" />
        ) : null}
